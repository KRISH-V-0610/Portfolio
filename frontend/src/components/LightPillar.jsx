...
const LightPillar = ({
  ...
  mixBlendMode = 'screen',
  pillarRotation = 0,
  cycleTopColor = false, // New prop for color cycling
  colorCycleSpeed = 0.002 // New prop for cycle speed
}) => {
  ...
  const colorCycleRef = useRef(0); // Track color cycle progress
  ...
  useEffect(() => {
    ...
    // rotationSpeed is handled in the loop via ref/prop closure? 
    // Actually, animate loop closes over initial props. 
    // We need to update a ref for rotationSpeed if we want it dynamic without re-init.
    // But for now, user set fixed speed.

  }, [topColor, bottomColor, intensity, glowAmount, pillarWidth, pillarHeight, noiseIntensity, pillarRotation]);

  if (!webGLSupported) {
    return (
      <div className={`w-full h-full absolute top-0 left-0 flex items-center justify-center bg-black/10 text-gray-500 text-sm ${className}`} style={{ mixBlendMode }}>
        WebGL not supported
      </div>
    );
  }
